...
namespace AndroidSideloader
{
    class SideloaderRCLONE
    {
        ...
        {
            RCLONE.runRcloneCommand($"sync \"{remote}:{RcloneGamesFolder}/.meta/notes\" \"{NotesFolder}\" --config .\\a");
        }

        public static void RefreshRemotes()
        {
            RemotesList.Clear();
            var remotes = RCLONE.runRcloneCommand("--config .\\a listremotes").Output.Split('\n');
        ...
        public static void initGames(string remote)
        {
            ...
            games.Clear();
            string tempGameList = RCLONE.runRcloneCommand($"cat \"{remote}:{RcloneGamesFolder}/GameList.txt\" --config .\\a").Output;
        ...
        {
            RCLONE.runRcloneCommand($"sync \"{remote}:{RcloneGamesFolder}/.meta/thumbnails\" \"{ThumbnailsFolder}\" --config .\\a");
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            dynamic results = JsonConvert.DeserializeObject<dynamic>(RCLONE.runRcloneCommand($"size \"{remote}:{RcloneGamesFolder}/{FolderName}\" --config .\\a --json").Output);
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            ...
            string hash = RCLONE.runRcloneCommand($"md5sum --config .\\a \"{remote}:Quest Homebrew/Sideloading Methods/1. Rookie Sideloader - VRP Edition/a\"").Output;
